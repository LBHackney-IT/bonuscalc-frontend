...
import { dateRegex, fetchOutOfHoursSummaries } from '@/utils/fetch'
...
export default authoriseAPIRequest(async (req, res) => {
  try {
    ...
    let output = 'No out-of-hours records for this week'

    if (!dateRegex.test(date)) {
      return res.status(StatusCodes.BAD_REQUEST).json(BAD_REQUEST_ERROR)
    }

    const summaries = await fetchOutOfHoursSummaries(date)
    ...
    res.setHeader(
      'content-disposition',
      `attachment; filename="out-of-hours-${date}.csv"`
    )
